const portfolioData = {
  name: "NAKUL B JAIN",
  bio: "I am a Computer Science engineering student driven by a deep passion to build innovative, practically feasible solutions. Whether I am crafting clean, responsive user interfaces or designing reliable backend systems, I enjoy turning complex ideas into functional code. For me, coding isn't just about writing code—it's a process of understanding problems and driving efficient solutions. With a strong foundation in core computer science principles and full-stack development, I enjoy solving real-world challenges, adapting to new technologies, and constantly improving my skills as a developer.",
  
  // FIXED SYNTAX ERROR HERE
  heroSocials: [
    { label: "LinkedIn", url: "https://www.linkedin.com/in/nakulbjain", icon: "fa-brands fa-linkedin" },
    { label: "GitHub", url: "https://github.com/nakulbjain", icon: "fa-brands fa-github" },
    { label: "CV", url: "assets/docs/nakulbjaincvmobi.pdf", icon: "fa-solid fa-file-lines" }
  ],
...
// Ensure DOM is ready before executing DOM-dependent scripts
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener('scroll', () => {
    ...
    if (scrollEl) scrollEl.style.width = scrolled + '%';
  });
  ...
  if (bioEl) bioEl.innerText = portfolioData.bio;
  
  const footerEl = document.getElementById("footer-text");
  if (footerEl) footerEl.innerText = `© ${new Date().getFullYear()} ${portfolioData.name}. All rights reserved.`;
  ...
  if (carouselWrapper && projectIndicators) {
    carouselWrapper.innerHTML = "";
    projectIndicators.innerHTML = "";
    portfolioData.projects.forEach((p, idx) => {
      carouselWrapper.innerHTML += `
        <div class="project-snap-card min-w-full">
    ...
    if (prevBtn) prevBtn.addEventListener('click', () => carouselWrapper.scrollBy({ left: -carouselWrapper.offsetWidth, behavior: 'smooth' }));
    if (nextBtn) nextBtn.addEventListener('click', () => carouselWrapper.scrollBy({ left: carouselWrapper.offsetWidth, behavior: 'smooth' }));
  }
